...
export default function WikiIndex({ project, pages }: Props) {
    ...
    return (
        <AppLayout title={`${project.key} — Wiki`} breadcrumbs={breadcrumbs}>
            <div className="mb-6">
                <ProjectTabs projectId={project.id} active="wiki" />
            </div>

            <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-text-strong">Wiki stránky</h2>
                <button
            ...
            <div className="rounded-lg border border-border-subtle bg-surface-primary">
                {pages.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center gap-3 py-12 text-text-muted">
                        ...
                        <p className="text-sm">Zatím žádné wiki stránky. Vytvořte první.</p>
                    </div>
                )}
            </div>
        </AppLayout>
