...
export default function AuditLogIndex({ entries, filters, actions, entityTypes, actors }: Props) {
    ...
    const applyFilter = useFilterRouter('/admin/audit-log');

    function toggleSort(field: string) {
        const dir = filters.sort === field && filters.dir !== 'desc' ? 'desc' : 'asc';
        router.get('/admin/audit-log', { ...filters, sort: field, dir }, { preserveState: true });
    }
    ...
    return (
        <AppLayout title="Audit log" breadcrumbs={BREADCRUMBS}>
            ...
            {/* Detail Modal */}
            <Modal open={!!selected} onClose={() => setSelected(null)} size="max-w-2xl" showClose={false}>
                {selected && (
                    <>
                        <h2 className="mb-4 text-lg font-semibold text-text-strong">Detail záznamu</h2>
