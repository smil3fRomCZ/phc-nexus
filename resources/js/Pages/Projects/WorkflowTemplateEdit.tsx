...
import { MousePointerClick, Plus, Shield, Trash2, X } from 'lucide-react';
...
function StatusNode({ data }: { data: TemplateStatus & { onEdit: (id: string) => void } }) {
    return (
        <div
...
export default function WorkflowTemplateEdit({ template }: Props) {
    const { statuses, transitions } = template;
    ...
    const breadcrumbs: Breadcrumb[] = [
        { label: 'Domů', href: '/' },
        { label: 'Šablony workflow', href: '/workflow-templates' },
        { label: template.name },
    ];

    const editingStatus = statuses.find((s) => s.id === editingId) ?? null;
    const base = `/workflow-templates/${template.id}`;
    ...
    const onConnect = useCallback(
        (connection: Connection) => {
            ...
            fetch(`${base}/transitions`, {
                method: 'POST',
                headers: csrfHeaders(),
                body: JSON.stringify({ from_status_id: connection.source, to_status_id: connection.target }),
            }).then((res) => {
    ...
    function confirmDeleteStatus() {
        ...
        fetch(`${base}/statuses/${deleteTarget.id}`, {
            method: 'DELETE',
            headers: csrfHeaders(),
        }).then((res) => {
            if (res.ok) {
    ...
    return (
        <AppLayout title={`${template.name} — Šablona`} breadcrumbs={breadcrumbs}>
            <div className="max-w-screen-xl">
                <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <h1 className="text-xl font-bold text-text-strong md:text-2xl">{template.name}</h1>
                        {template.is_system && (
                            <span className="flex items-center gap-1 rounded-full bg-brand-soft px-2 py-0.5 text-xs font-bold text-brand-primary">
                ...
                {template.description && <p className="mb-4 text-sm text-text-muted">{template.description}</p>}
                ...
                <div className="mb-4 flex flex-wrap items-center gap-3 rounded-lg border border-border-subtle bg-surface-secondary px-4 py-2">
                    <input
                        ...
                        className="w-full rounded-md border border-border-default bg-surface-primary px-3 py-1.5 text-sm focus:border-border-focus focus:outline-none focus:shadow-[0_0_0_2px_var(--color-brand-soft)] sm:w-48"
                    />
                    <button
                        onClick={addStatus}
                        disabled={!newName}
                        ...
                        {editingStatus ? (
                            <>
                                ...
                                <div className="space-y-3">
                                            ...
                                            {[
                                                ...
                                                >
                                                    <input
                                                        type="checkbox"
                                                        checked={
                                                            editingStatus[flag.key as keyof TemplateStatus] as boolean
                                                        }
                                                        onChange={(e) =>
                                    ...
                                    <DeleteButton
                                        onClick={() =>
                                            setDeleteTarget({ id: editingStatus.id, name: editingStatus.name })
                                        }
            ...
            <ConfirmModal
                ...
                variant="danger"
                title="Smazat stav"
                message={`Smazat stav „${deleteTarget?.name}" ze šablony?`}
                confirmLabel="Smazat"
