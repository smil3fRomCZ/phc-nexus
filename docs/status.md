...
| — | Security Audit — Sprint 5 — PR3 (C2 + M10) | **DONE** | Invitation integrity — email match + expiration check v AuthenticateGoogleUser, zabráněno overwrite existingRole, tokeny v DB jen jako SHA256 (backfill migrace, `token` sloupec dočasně zachován pro BC). 5 regresních testů (token swap, expired, existing user role, case insensitive email). |
...
| — | Security Audit — Sprint 7 — PR8 (H8 + H4) | **DONE** | H8 Session fixation — `$request->session()->regenerate()` po `Auth::login()` v `GoogleAuthController`. H4 Wiki parent_id IDOR — `Rule::exists` scoped na `project_id`/`epic_id`, `whereNot` pro self-reference, bonus route-level IDOR fix (`abort(404)` pokud wiki page nepatří do projektu/epicu z URL). 11 regresních testů. |
