...
| 2026-04-13 | Projects | Granulární role členů projektu — `ProjectRole` enum (Admin/Contributor/Viewer). Owner = implicitní Admin. Policies (Project/Epic/Task) rozlišují: Viewer pouze read, Contributor CRUD úkolů/epiků/času, Admin + správa projektu a členů. Migrace `project_manager`→`admin`, `member`→`contributor`. UI Members + MemberUsageModal sladěny — PR #152 |
